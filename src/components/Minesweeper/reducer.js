...
export const reducer = handleActions({
  NEW_GAME: (state, action) => {
    ...
    return {
      ...
      squares: new Board(settings.height, settings.width, settings.mineCount)
        .squares
    };
  },
...
function select(state) {
  return {
    ...
    isEverySafeSquareRevealed: () =>
      state.revealed.every((isRevealed, id) =>
        state.squares[id].mine ? !isRevealed : isRevealed
      )
  };
}
...
function explodeAndRevealMines(state, id) {
  ...
  state.squares
    ...
    .forEach((mine) => (revealed[mine.id] = true));
  return {
    ...state,
    revealed,
    exploded: id
...
function winGame(state) {
  ...
  state.squares
    .filter((square) => square.mine)
    .forEach((mine) => (flags[mine.id] = true));
