...
const StyledTask = styled(TaskbarButton)`
  ...
  box-shadow: ${(props) =>
    props.isActive
      ? taskActiveBoxShadow(props.theme)
      : taskBoxShadow(props.theme)};
  outline: 0;
  ...
  .isLeftClicking &:active:hover {
    box-shadow: ${(props) =>
      props.isActive
        ? taskActiveBoxShadow(props.theme)
        : taskClickedBoxShadow(props.theme)};
  }
`;

const Task = React.forwardRef(({ id, title, icon }, ref) => {
  const { activeTask, setTaskActiveStatus } = useTaskManager();
  const [isBeingClickedWhileActive, setIsBeingClickedWhileActive] = useState(
    false
  );
  ...
  useEventListener(ref, 'mousedown', () => {
    ...
    document.addEventListener(
      'mouseup',
      () => setIsBeingClickedWhileActive(false),
      { once: true }
    );
  });
  ...
  return (
    <StyledTask ref={ref} isActive={isCurrentlyActive}>
      {icon ? <TaskbarIcon src={icon} /> : ''}{' '}
      <Text bold={isCurrentlyActive}>{title}</Text>
    </StyledTask>
  );
});
